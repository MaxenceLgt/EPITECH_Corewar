...
static void count_bytes_direct(size_t pos_lab, size_t my_pos, char **lines,
int fd_out)
{
    u_int32_t res = 0;
    ...
    for (size_t i = my_pos - 1; i >= pos_lab; i--)
        res -= calculate_size_cmd(lines[i]);
    if (res != 0) {
        change_endians(&res, sizeof(u_int32_t));
        write(fd_out, &res, 4);
        return;
    }
    for (size_t i = my_pos; i < pos_lab; i++)
...
void transcribe_direct_label(compiler_t *info, size_t pos, char *name)
{
    ...
    for (size_t i = pos; i >= 2; i--) {
        ...
        if (ml_strncmp(temp[0], name, name_l) == 0 && temp[0][name_l] == ':') {
            ml_destroy_str_array(temp);
            return (count_bytes_direct(i, pos, info->f_lines, info->fd_out));
        }
        ml_destroy_str_array(temp);
    }
    for (size_t i = pos; info->f_lines[i]; i++) {
